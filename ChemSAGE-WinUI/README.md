...
Phase 2 adds production-oriented Resource Vault and Past Papers Archive surfaces on top of the Phase 1 shell:

- **Resource Vault**: Supabase-backed resource listing, server-side search/filter query construction, course/semester/type filters, selected-resource metadata preview through cards, upload through native Windows file picker, Supabase Storage upload, download through Supabase Storage, transfer progress, loading state, empty-state messaging, and error handling.
- **Past Papers Archive**: Supabase-backed paper browsing, subject/semester/year/exam-type filtering, search, metadata preview cards, year-descending sorting, Storage downloads, transfer progress, loading state, empty-state messaging, and error handling.
- **Shared controls**: reusable `LoadingView`, `ErrorView`, `EmptyStateView`, `SearchBar`, `FilterPanel`, `ResourceCard`, and `PaperCard` controls under `Components/`.
- **Architecture**: page code-behind only initializes XAML and binds view models; data access, filtering, upload/download orchestration, and progress updates live in async MVVM services/view models with `CancellationToken` support.
...
3. **Phase 3: Advanced content workflows** — add offline-safe upload queues, resource/paper version history, comment threads, starring, previews, and Supabase Storage policy validation tooling.
