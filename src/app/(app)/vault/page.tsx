...
export default function StudyVaultPage() {
  ...
  if (profile.status !== "active") {
    return <LockedScreen title="Study Vault locked" description="Only active users can browse the shared study vault." />;
  }

  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader title="Study Vault" description="Browse live resources from Supabase Storage and filter them instantly on the client." profile={profile} action={profile.role === "admin" ? <label className="inline-flex cursor-pointer items-center gap-2 rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white"><Upload size={16} /> Upload<input type="file" className="hidden" onChange={async (event) => {
      ...
      <div className="mb-6 grid gap-4 rounded-3xl border border-slate-800 bg-slate-900/60 p-5 md:grid-cols-[1fr_auto]">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search resources by title..." className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white outline-none ring-0 placeholder:text-slate-500 focus:border-blue-500" />
        <div className="flex flex-wrap gap-2">
          {categories.map((item) => (
            <button key={item} onClick={() => setCategory(item)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${category === item ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}>
              {item}
            </button>
          ))}
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {filteredResources.map((resource) => (
          <article key={resource.id} className="rounded-3xl border border-slate-800 bg-[#0f172a]/80 p-5">
            <div className="mb-4 flex items-start justify-between gap-3">
              ...
              <span className="rounded-full bg-slate-800 px-3 py-1 text-[11px] font-semibold text-slate-400">{resource.file_type || "file"}</span>
            </div>
            <p className="mb-4 text-sm text-slate-400">Uploaded {formatDateTime(resource.created_at)}</p>
            <a href={resource.file_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-4 py-2 text-sm font-semibold text-emerald-300 transition hover:bg-emerald-500/20">
